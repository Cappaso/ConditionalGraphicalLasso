close all; clear;
% experiment script for corr logi
...
for iTune1 = 1:length(rho1)
    for iTune2 = 1:length(rho2)
		...
		% CGL solved by pseudo-likelihood
        [B,A] = cond_graph_lasso(featr,gndtr,rho1(iTune1),rho2(iTune2));
        Yprd = Prd_CGL_LBP(feats,B,A);
        
%         % CGL solved by mean-field inference
%         [B,A] = CGL_Learning(featr,gndtr,rho1(iTune1),rho2(iTune2));
%         Yprd = double(CGL_Inference(feats,B,A)>0.5);
