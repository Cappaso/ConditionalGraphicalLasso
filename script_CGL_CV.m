...
for kk = 1:numExp
    ...
    switch methodName
        case 'CGL'
            disp('Run CGL sovled by pseudo-likelihood...');
            [ label_pred{kk}, run_time{kk} ] = Lite_CGL(data_train, label_train, data_test, label_test);
        case 'mfCGL'
            disp('Run CGL solved by mean-field inference...');
            [ label_pred{kk}, run_time{kk} ] = Lite_mfCGL(data_train, label_train, data_test, label_test);
        otherwise
